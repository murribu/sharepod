...
class User extends SparkUser
{
    ...
    public function liked_episodes(){
        ...
        $episodes = Episode::leftJoin('shows', 'shows.id', '=', 'episodes.show_id')
                ...
                ->selectRaw('likes.created_at as likeddate, episodes.name, episodes.slug, episodes.description, episodes.pubdate, episodes.id, episodes.img_url, episodes.show_id, shows.name as show_name, shows.slug as show_slug, ae.result_slug, count(distinct total_likes.id) as total_likes, count(distinct this_user_likes.id) as this_user_likes, count(distinct recommendations.id) total_recommendations, count(distinct pe.playlist_id) total_playlists, count(distinct ae.id) this_user_archived')
    ...
    public function archived_episodes(){
        ...
        $episodes = Episode::join(DB::raw('(select archived_episodes.id, url, slug, filesize, result_slug, episode_id from archived_episodes inner join archived_episode_users on archived_episodes.id = archived_episode_users.archived_episode_id where (result_slug is null or result_slug = \'ok\') and user_id = '.$user->id.' and active = 1) ae'), 'ae.episode_id', '=', 'episodes.id')
            ...
            ->leftJoin('playlist_episodes as pe', 'pe.episode_id', '=', 'episodes.id')
            ->leftJoin('recommendations', 'recommendations.episode_id', '=', 'episodes.id')
            ->selectRaw('episodes.show_id, episodes.id, episodes.slug, episodes.name, episodes.description, episodes.duration, episodes.explicit, coalesce(ae.url, concat(\''.env('S3_URL').'/'.env('S3_BUCKET').'/episodes/\', ae.slug, \'.mp3\'), episodes.url) url, coalesce(ae.filesize, episodes.filesize) filesize, episodes.img_url, episodes.pubdate, ae.result_slug, count(distinct total_likes.id) as total_likes, count(distinct this_user_likes.id) as this_user_likes, count(distinct recommendations.id) total_recommendations, count(distinct pe.playlist_id) total_playlists, count(distinct ae.id) this_user_archived')
