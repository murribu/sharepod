...
class User extends SparkUser
{
    ...
    public function liked_episodes(){
        ...
        $episodes = Episode::leftJoin('shows', 'shows.id', '=', 'episodes.show_id')
                ...
                ->selectRaw('likes.created_at as likeddate, episodes.name, episodes.slug, episodes.description, episodes.pubdate, episodes.id, shows.name as show_name, shows.slug as show_slug, ae.result_slug, count(distinct total_likes.id) as total_likes, count(distinct this_user_likes.id) as this_user_likes, count(distinct recommendations.id) total_recommendations, count(distinct pe.playlist_id) total_playlists, count(distinct ae.id) this_user_archived')
