...
trait HasSlug{
    ...
    public static function findSlug($slug = false){
        ...
        $slug_length = property_exists( new self, 'slug_length') ? self::$slug_length : 32;
        if ($slug){
            $slug = substr(strtolower(preg_replace("/[^a-zA-Z\d]/", "-", $slug)), 0, ($limit - 5));
        }else{
            $slug = self::generateRandomString($slug_length);
        }
    ...
    public static function findSlugWithPrefix($prefix){
        ...
        $slug = substr($prefix . self::generateRandomString($slug_length - strlen($prefix)), 0, ($limit - 5));
        $exists = self::where($field, $slug)->first();
        if (!$exists && !in_array($slug, $reserved_words)){
            return $slug;
        }
        $i = 0;
        while ($exists || in_array($slug, $reserved_words)){
            $exists = self::where($field, substr($prefix . self::generateRandomString($slug_length - strlen($prefix)), 0, ($limit - 5)))->first();
        }
        return $slug."-".$i;
    }
